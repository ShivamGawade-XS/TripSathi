...
const navItems = [
  ...
  { label: "Saved", href: "/dashboard", icon: "M5 5a2 2 0 012-2h10a2 2 0 012 2v16l-7-3.5L5 21V5z" },
  ...
  return (
    <nav className="md:hidden fixed bottom-0 left-0 right-0 z-50 bg-white border-t border-surface-200 safe-area-bottom">
      <div className="flex items-center justify-around py-2">
        {navItems.map((item) => (
          <Link key={item.href} href={item.href} className={`flex flex-col items-center gap-0.5 px-3 py-1.5 rounded-lg transition-colors ${pathname === item.href ? "text-primary-600" : "text-surface-400 hover:text-surface-600"}`}>
